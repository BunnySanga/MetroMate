...
void init_delhi_graphs() {
    ...
    if (!stations_file.is_open()) {
        std::cerr << "Error opening delhi/stations.txt" << std::endl;
        return;
    }
    std::string line;
    ...
    if (!connections_file.is_open()) {
        std::cerr << "Error opening delhi/connections.txt" << std::endl;
        return;
    }
...
int calculate_fare(float total_distance) {
    ...
    else if (total_distance <= 21) return 40;
    else if (total_distance <= 32) return 50;
    else return 60;
}
