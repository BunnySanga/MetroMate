...
void init_hyderabad_graphs() {
    ...
    if (!stations_file.is_open()) {
        std::cerr << "Error opening hyderabad/stations.txt" << std::endl;
        return;
    }
    std::string line;
    ...
    if (!connections_file.is_open()) {
        std::cerr << "Error opening hyderabad/connections.txt" << std::endl;
        return;
    }
...
int calculate_fare(float total_distance) {
    // Simplified fare structure based on distance (similar to Delhi, adjust as needed)
