...
void init_intercity_graphs() {
    std::cout << "Opening intercity/stations.txt...\n";
    std::ifstream stations_file("intercity/stations.txt");
    if (!stations_file.is_open()) {
        std::cerr << "Failed to open intercity/stations.txt\n";
        return;
    }
    ...
    stations_file.close();
    std::cout << "Finished reading intercity/stations.txt. Max ID: " << max_id << "\n";
    ...
    std::cout << "Opening intercity/connections.txt...\n";
    std::ifstream connections_file("intercity/connections.txt");
    if (!connections_file.is_open()) {
        std::cerr << "Failed to open intercity/connections.txt\n";
        return;
    }
    ...
    std::cout << "Finished reading intercity/connections.txt\n";
}

float calculate_fare(float total_distance) {
    if (total_distance <= 50) return 50.0;  // Base fare for short inter-city trips
    else if (total_distance <= 200) return 100.0;  // Medium distance
    else if (total_distance <= 500) return 200.0;  // Long distance
    else return 300.0;  // Very long distance
}

std::string get_line_name(char line_color) {
    switch (line_color) {
        case 'i': return "Intercity Line";
...
void find_intercity_route(const std::string& source, const std::string& dest, int choice) {
    ...
    for (size_t i = 0; i < path.size() - 1; ++i) {
        ...
        if (line_color != 't') line_sequence.push_back(line_color);
    }
    segments.emplace_back(current_segment, current_line);

    int interchanges = 0;
    for (size_t i = 1; i < line_sequence.size(); ++i) {
        if (line_sequence[i] != line_sequence[i - 1] && line_sequence[i] != 't' && line_sequence[i - 1] != 't') {
            interchanges++;
        }
    }
    ...
    for (size_t i = 0; i < segments.size(); ++i) {
        ...
        if (i < segments.size() - 1 && segments[i + 1].second != 't' && segments[i].second != segments[i + 1].second) {
