...
void init_intercity_graphs() {
    ...
    if (!stations_file.is_open()) {
        std::cerr << "Error opening inter-city/stations.txt" << std::endl;
        return;
    }
    std::string line;
    ...
    if (!connections_file.is_open()) {
        std::cerr << "Error opening inter-city/connections.txt" << std::endl;
        return;
    }
...
int calculate_fare(float total_distance) {
    // Simplified inter-city fare structure (adjust for hypothetical subway fares)
    ...
    else return 2000;  // Max fare for very long distances
}
...
void find_intercity_route(const std::string& source, const std::string& dest, int choice) {
    ...
    std::cout << "Inter-city route from " << source << " to " << dest << ":" << std::endl;
    for (int id : path) {
