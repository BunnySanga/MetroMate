...
namespace intercity {
    ...
    void init_intercity_graphs() {
        ...
        if (!stations_file.is_open()) {
            std::cerr << "Error: Failed to open inter-city/stations.txt. Check file path and permissions.\n";
            return;
        }
        ...
        if (!connections_file.is_open()) {
            std::cerr << "Error: Failed to open inter-city/connections.txt. Check file path and permissions.\n";
            return;
        }
    ...
    void find_intercity_route(const std::string& source, const std::string& dest, int choice) {
        if (station_map.find(source) == station_map.end() || station_map.find(dest) == station_map.end()) {
            std::cout << "Station not found. Available stations:\n";
